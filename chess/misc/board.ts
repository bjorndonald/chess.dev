...
export default function evaluateBoard(move: Move, prevSum: number, color: Color) {
    var from = [8 - parseInt(move.from[1]), move.from.charCodeAt(0) - 'a'.charCodeAt(0)];
    var to = [8 - parseInt(move.to[1]), move.to.charCodeAt(0) - 'a'.charCodeAt(0)];

    // Change endgame behavior for kings
    if (prevSum < -1500) {
        if (move.piece === 'k') { move.piece = 'k_e' as PieceSymbol }
        else if (move.captured === 'k') { move.captured = 'k_e' as PieceSymbol }
    }
